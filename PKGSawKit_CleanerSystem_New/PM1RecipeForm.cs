...
{
    public partial class PM1RecipeForm : UserControl
    {
        ...
        public PM1RecipeForm(RecipeForm parent)
        {
            m_Parent = parent;
        ...
        private void PM1RecipeForm_Load(object sender, EventArgs e)
        {
            Width = 1172;
            Height = 824;
            Top = 0;
        ...
        private void Get_lstRecipeFile()
        {
            try
            {
                ...
                if (Directory.Exists(string.Format("{0}PM1", Global.RecipeFilePath)))
                {
                    string[] FileList = Directory.GetFiles(string.Format("{0}PM1", Global.RecipeFilePath), "*.csv");
        ...
        {            
            strFileName = string.Format("{0}PM1\\{1}", Global.RecipeFilePath, strFileName);
            ...
            try
            {
                ...
                if (e.RowIndex >= 0 && recipeGrid.Rows.Count - 1 >= e.RowIndex && e.ColumnIndex > 0)
                {
                    ...
                    if (m_nEditMode == (byte)RecipeEditMode.EDIT_MODE)
                    {
                        ...
                        else if ((e.RowIndex >= 1) && (e.RowIndex <= 3))
                        {
        ...
        private void btn_RecipeFile_New_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (KeyDlg.ShowDialog() == DialogResult.OK)
                {
                    ...
                    string RecipePath = string.Format("{0}PM1\\", Global.RecipeFilePath);                    
